...
Procedure run() Экспорт
  ...
  фл = Новый Файл(log_file);
	ПеренестиЛоги(фл.Путь);
EndProcedure // run()

Процедура ПеренестиЛоги(КаталогЛогов)
...
Процедура УбедитьсяВНаличииРабочихКаталогов()
  ...
  Если Не КаталогБэкапа.Существует() Тогда
    obj.lg("Не обнаружен каталог для сохранения dt файлов "+КаталогБэкапа.ПолноеИмя);
  КонецЕсли;
  КаталогВыгрузки = Новый Файл("dt");
  Если Не КаталогВыгрузки.Существует() Тогда
    obj.lg("Не обнаружен каталог dt файлов "+КаталогВыгрузки.ПолноеИмя, 3);
    СоздатьКаталог(КаталогВыгрузки.ПолноеИмя);
  КонецЕсли;
  КаталогПротоколов =  Новый Файл("log");
  Если Не КаталогПротоколов.Существует() Тогда
    obj.lg("Не обнаружен каталог файлов протоколов "+КаталогПротоколов.ПолноеИмя, 3);
